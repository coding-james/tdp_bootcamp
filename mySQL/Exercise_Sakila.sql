...
SELECT r.rental_date, r.return_date, i.film_id, (DATE_ADD(r.rental_date, INTERVAL (SELECT rental_duration FROM film f WHERE f.title="Academy Dinosaur") DAY)) AS due_date FROM rental r JOIN inventory i ON r.inventory_id=i.inventory_id WHERE i.film_id=(SELECT DISTINCT f.film_id FROM film f JOIN inventory i ON f.film_id=i.film_id WHERE f.title="Academy Dinosaur") ORDER BY return_date ASC LIMIT 1;
# When was the rental due back 27-08-2022
